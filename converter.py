...
class UniversalDependencyGraph(DependencyGraph):
    ...
    def to_conllU(self):
        ...
        return '# ' + self.sent_id + '\n' \
               + ''.join(template.format(i=i, **node, deps_str=self._deps_str(node['deps']))
...
class Converter():
    ...
    def _conllU_tag(tag):
        """
            ...
            http://www.linguist.is/icelandic_treebank/Head_Types
            to
        ...
        #todo base conversion on http://code.google.com/p/universal-pos-tags

        map = {
            'ADJ': 'ADJ',
            #'': 'ADP',
            'ADV': 'ADV',
            #BE VERA (BE verb); DO GERA (DO verb); HV HAFA (HAVE verb); MD modal verb; RD
            #+past participle: VBN, BEN, DON, HVN, RDN
            'BE': 'AUX', 'DO': 'AUX', 'HV': 'AUX', 'MD': 'AUX', 'RD': 'AUX',
            #VAN, BAN, DAN, HAN
            'VAN': 'AUX', 'BAN': 'AUX', 'DAN': 'AUX', 'HAN': 'AUX',
            'D': 'DET',
            #'': 'INTJ',
            'N': 'NOUN',
            'NUM': 'NUM',
        ...
        for (m, utag) in OrderedDict(sorted(map.items(), key=lambda x: -len(x[0]))).items():
            if tag.startswith(m):
                return utag
        ...
        for i in t.treepositions():
            if isinstance(t[i], Tree):
                ...
                self.dg.add_node({'address': nr, 'word': form, 'lemma': lemma,
                                  'ctag': self._conllU_tag(tag_list[nr]), 'tag': tag_list[nr],
